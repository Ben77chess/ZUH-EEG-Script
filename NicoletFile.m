classdef NicoletFile < handle
  ...
  %This version slightly modified by Benjamin Weinberg in Fall 2018 to work
  %with other scripts (marked by comments)
...
%       reserved = fread(h, 3, 'uint16'); %#ok<NASGU>
      ...
      if length(tsPackets) > 1
        ...
        %Problem here with multiple TSinfo packets. Modified to continue anyways. Benjamin Weinberg
      ...
      HCEVENT_EYESCLOSED        =  '{F1DE7B01-688B-46CA-9455-67E3A9337A5C}';
      HCEVENT_EYESOPEN          =  '{6C878BEC-9DEF-4020-B013-1AFFBE4464BB}';
      HCEVENT_BLINK             =  '{A5A95603-A7F8-11CF-831A-0800091B5BDA}';
      HCEVENT_DEYESCLOSED1      =  '{A5A95601-A7F8-11CF-831A-0800091B5BDA}';
      ...
      while (pktGUID == evtPktGUID)
          ...
          switch obj.eventMarkers(i).GUID
              ...
              case HCEVENT_EYESCLOSED
                  obj.eventMarkers(i).IDStr = 'Eyes Closed';
              case HCEVENT_DEYESCLOSED1
                  obj.eventMarkers(i).IDStr = 'Eyes Closed';
              case HCEVENT_EYESOPEN
                  obj.eventMarkers(i).IDStr = 'Eyes Open';
              case HCEVENT_BLINK
                  obj.eventMarkers(i).IDStr = 'Blink';
              otherwise
    ...
    function out = getSampleRate(obj, segment, chidx) %Benjamin Weinberg
      % GETSAMPLERATE  Returns the sample rate in samples per second per channel.
      %
      %   OUT = GETSAMPLERATE(OBJ, SEGMENT) returns a 1xn array of values
      %   indicating the sample rate for each of the channels in the
      %   associated SEGMENT, where SEGMENT is the index of the
      %   OBJ.segments array.
      ...
      out = obj.segments(segment).samplingRate(chidx);
      
    end
    
    function out = getDuration(obj, segment) %Benjamin Weinberg
      % GETNRSAMPLES  Returns the number of samples per channel in segment.
      %
      %   OUT = GETNRSAMPLES(OBJ, SEGMENT) returns a 1xn array of values
      %   indicating the number of samples for each of the channels in the
      ...
      out = obj.segments(segment).duration;
